...
public class Constraint
{
	...
	public Constraint(double[] a_k, IConstrainedDouble[] a_v, Type[] a_t, double a_c, Operator a_op)
	{
		this (a_k, a_v, a_t, a_c, a_op, ClStrength.required);
	}
	
	public Constraint(double[] a_k, IConstrainedDouble[] a_v, Type[] a_t, double a_c, Operator a_op, ClStrength a_strength)
	{
		for (IConstrainedDouble v : a_v) assert v != null;
		_c = a_c;
		_k = a_k;
		_op = a_op;
		_t = a_t;
	...
	public IConstrainedDouble[] get_v()
	{
		return _v;
	}
	...
	{
		double tC = _c; // total k term, after simplifying constants.
		
		ClLinearExpression expr = new ClLinearExpression();
		for(int i=0;i<_k.length;i++)
		{
			double k = _k[i];
			IConstrainedDouble v = _v[i];
			Type t = _t[i];
			
			if (t == Type.CONST) tC += k * v.get();
			else if (t == Type.VAR) expr.addVariable(v, k); 
		}
		expr.set_constant(tC);
		
		switch (_op)
		{
		case EQ: return new ClLinearEquation(expr, _strength);
		case GEQ: return new ClLinearInequality(expr, _strength);
		case LEQ: return new ClLinearInequality(expr.multiplyMe(-1), _strength);
